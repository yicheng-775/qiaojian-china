...
QJC.segments = (function () {
  ...
  function splitText(text, maxLen, langDir) {
    if (!text) return [];
    if (text.length <= maxLen) return [text];
    var punct = langDir === "en2zh" ? "[^。！？!?；;.]+[。！？!?；;.]?" : "[^。！？!?；;]+[。！？!?；;]?";
    var sentences = text.match(new RegExp(punct, "g")) || [text];
    var chunks = [], cur = "";
  ...
  function splitSegments(text, langDir) {
    var raw = splitBlocks(text, langDir);
  ...
  function splitBlocks(text, langDir) {
    if (!text) return [];
    var t = text.trim();
    if (!t) return [];
    // 1) 空行切分
    ...
    // 3) 句末标点切分（尽量每段 1–2 句；英译中按英文句点切）
    return splitText(t, 120, langDir);
  }
